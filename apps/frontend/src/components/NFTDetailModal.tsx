  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 animate-fade-in" role="dialog">
      ...
      <div className="relative z-10 w-full max-w-4xl max-h-[92vh] overflow-y-auto rounded-3xl border border-[#2a2a3a] bg-[#12121a]/95">
        <button type="button" onClick={onClose} className="absolute top-4 right-4 z-20 h-9 w-9 rounded-full border border-[#2a2a3a]">
          ✕
        </button>
        <div className="grid md:grid-cols-2">
          ...
          <div className="flex flex-col gap-3 p-6">
            ...
            {(txMsg || error || lastTx) && (
              <p className="text-[11px] text-amber-200">{txMsg || error}{lastTx ? ` · ${lastTx}` : ''}</p>
            )}

            <div className="mt-auto flex flex-col gap-2 pt-2">
              <a href={XOXNO_COLLECTION(nft.collection)} target="_blank" rel="noreferrer" className="btn-secondary text-center text-sm">
                XOXNO ↗
              </a>
              <a href={EXPLORER_NFT(nft.identifier)} target="_blank" rel="noreferrer" className="btn-secondary text-center text-sm">
                Explorer ↗
              </a>
              <a href={LINKS.xexchangeTroUsdc} target="_blank" rel="noreferrer" className="btn-secondary text-center text-sm">
                $TRO ↗
              </a>
            </div>
          </div>
