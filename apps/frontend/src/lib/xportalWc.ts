/**
 * xPortal / WalletConnect V2 — mainnet login.
 * Retourne l’adresse erd1 après approval dans xPortal.
 */
import { XPORTAL_DEEP_LINKS, sdkDappConfig, WALLET_CONNECT_V2_RELAY_URL } from '../config/sdkDapp'

const MAINNET = '1'

export type XPortalLoginProgress = {
  phase: 'init' | 'uri' | 'waiting' | 'done' | 'error'
  uri?: string
  message?: string
}

function openXPortalWithUri(uri: string) {
  try {
    const encoded = encodeURIComponent(uri)
    const deep =n      XPORTAL_DEEP_LINKS.walletConnectUri?.(uri) ||
      `https://maiar.page.link/?apn=com.elrond.maiar.wallet&isi=1519405832&ibi=com.elrond.maiar.wallet&link=${encodeURIComponent(
        `https://xportal.com/?wallet-connect=${encoded}`,
      )}`
    window.open(deep, '_blank', 'noopener,noreferrer')
    // Tentative scheme natif (mobile)
    try {
      window.location.href = `xportal://wc?uri=${encoded}`
    } catch {
      /* ignore */
    }
  } catch {
    /* ignore */
  }
}

/**
 * Login xPortal mainnet via WC v2.
 * onProgress: UI QR / statut.
 */
export async function loginWithXPortalMainnet(
  onProgress?: (p: XPortalLoginProgress) => void,
): Promise<{ ok: true; address: string } | { ok: false; error: string }> {
  const projectId = sdkDappConfig.walletConnectV2ProjectId
  if (!projectId || projectId.length < 32) {
    return { ok: false, error: 'WalletConnect projectId manquant (VITE_WALLETCONNECT_PROJECT_ID).' }
  }

  onProgress?.({ phase: 'init', message: 'Initialisation WalletConnect…' })

  try {
    const mod = await import('@multiversx/sdk-wallet-connect-provider')
    const WalletConnectV2Provider =
      (mod as { WalletConnectV2Provider?: new (...args: unknown[]) => XcProvider }).WalletConnectV2Provider ||
      (mod as { default?: { WalletConnectV2Provider?: new (...args: unknown[]) => XcProvider } }).default
        ?.WalletConnectV2Provider

    if (!WalletConnectV2Provider) {
      return {
        ok: false,
        error: 'sdk-wallet-connect-provider indisponible. Utilise Web Wallet en attendant.',
      }
    }

    let resolvedAddress: string | null = null

    const callbacks = {
      onClientLogin: async () => {
        /* handled after approval */
      },
      onClientLogout: async () => {
        /* no-op */
      },
      onClientEvent: async () => {
        /* no-op */
      },
    }

    const provider = new WalletConnectV2Provider(
      callbacks,
      MAINNET,
      WALLET_CONNECT_V2_RELAY_URL,
      projectId,
    ) as XcProvider

    await provider.init()
    const { uri, approval } = await provider.connect()

    if (uri) {
      onProgress?.({ phase: 'uri', uri, message: 'Ouvre xPortal et approuve la session' })
      openXPortalWithUri(uri)
    }

    onProgress?.({ phase: 'waiting', uri, message: 'En attente d’approbation xPortal…' })
    await approval()

    const address =
      typeof provider.getAddress === 'function'
        ? await provider.getAddress()
        : (provider as { address?: string }).address

    resolvedAddress = (address || '').trim()
    if (!resolvedAddress || !/^erd1[a-z0-9]{58}$/i.test(resolvedAddress)) {
      return { ok: false, error: 'Adresse xPortal invalide après login.' }
    }

    onProgress?.({ phase: 'done', message: resolvedAddress })
    return { ok: true, address: resolvedAddress }
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e)
    onProgress?.({ phase: 'error', message: msg })
    return {
      ok: false,
      error:
        msg.includes('User rejected') || msg.includes('rejected')
          ? 'Connexion refusée dans xPortal.'
          : `xPortal WC: ${msg}. Essaie Web Wallet.`,
    }
  }
}

type XcProvider = {
  init: () => Promise<boolean>
  connect: () => Promise<{ uri?: string; approval: () => Promise<unknown> }>
  getAddress?: () => Promise<string>
  address?: string
  logout?: () => Promise<void>
}
