...
/// Invalid is not considered a valid vote thus is being completely
...
/// Voting module that handles proposal creation, voting, and resolution
/// Implements the core governance functionality including:
/// - Proposal creation and lifecycle management
/// - Vote casting and power calculation
/// - Proposal status tracking and resolution
#[multiversx_sc::module]
pub trait VotingModule: crate::storage::StorageModule + crate::events::EventsModule {
    /// Creates a new proposal for governance voting
    /// Process:
    ...
    /// Gets the status of a proposal at the current time
    /// Proposals have a lifecycle: Pending → Active → Approved/Rejected/Failed
    /// A proposal is approved if approve votes > reject votes (and total votes meet minimum)
    ...
    /// Gets the voting power for a user on a specific proposal
    /// Voting power is calculated as: staked_TRO + sum(staked_LP_i * LP_to_TRO_ratio_i)
    /// The LP-to-TRO ratio determines how much voting power each LP token provides
