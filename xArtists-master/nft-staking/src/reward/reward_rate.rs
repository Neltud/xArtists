...
pub trait RewardRateModule {
    ...
    fn handle_increase_reward_rate(&self, payment: &EsdtTokenPayment) {
        self.last_reward_rate_update(&payment.token_identifier)
            .set(self.blockchain().get_block_round());
    ...
    fn handle_increase_reward_rate_raw(&self, token_id: &TokenIdentifier, amount: BigUint) {
        ...
        self.last_reward_rate_update(token_id)
            .set(self.blockchain().get_block_round());
    }
    ...
    fn get_unstored_rewards_for_token(
        ...
        // if the user has no reward rate set, they start from the current reward rate. This should ensure new
        // stakers will receive no past rewards but will continue accruing rewards from that point on.
        ...
        let user_rate = match self.user_reward_rate(user, reward_token_id).is_empty() {
            true => {
                let last_user_update = self.last_user_update(user).get();
                let last_reward_rate_update = self.last_reward_rate_update(reward_token_id).get();
                if last_user_update > last_reward_rate_update {
                    current_rate.clone()
                } else {
                    BigUint::zero()
                }
    ...
    #[view(getLastUserUpdate)]
    #[storage_mapper("lastUserUpdate")]
    fn last_user_update(&self, user: &ManagedAddress) -> SingleValueMapper<u64>;

    #[view(getLastRewardRateUpdate)]
    #[storage_mapper("lastRewardRateUpdate")]
    fn last_reward_rate_update(&self, reward_token_id: &TokenIdentifier) -> SingleValueMapper<u64>;
}
