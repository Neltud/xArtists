...
pub trait PlannedDistributionModule: super::reward_rate::RewardRateModule {
    ...
    fn get_user_undistributed_rewards_share(
        ...
        let total_score = self.aggregated_staked_score().get();
        let user_score = self.user_staked_score(user).get();

        let mut adjusted_rewards = ManagedVec::new();

        for reward in undistributed_rewards.iter() {
            let mut adjusted_reward = reward.clone();
            adjusted_reward.amount = adjusted_reward.amount * &user_score / &total_score;
    ...
    fn get_user_undistributed_rewards_share_for_token(
        &self,
        user: &ManagedAddress,
        token_id: &TokenIdentifier,
    ) -> Option<EsdtTokenPayment<Self::Api>> {
        let undistributed_rewards = self.get_user_undistributed_rewards_share(user);
        if undistributed_rewards.is_empty() {
            return None;
        }
